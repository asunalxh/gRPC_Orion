...
bool RocksDBConnector::Get(const void *key, int key_len, void *value)
{
	...
	string value_str;
	bool flag = this->GetValue(key_str, &value_str);
...
{
	return this->PutValue(Slice((const char *)key, key_len), Slice((const char *)value, value_len));
}

bool RocksDBConnector::GetValue(Slice key, string *value)
{
...
bool RocksDBConnector::PutValue(Slice key, Slice value)
{
